			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						$oOnlineBetaalPlatform->setPaymentMethod('paypal');
						...
						if(!empty($this->oRecord['order_params']))
						{
							...
							{
								$aCustomerData['buyer_name_first'] = substr($aOrderParams['customer']['shipment_first_name'], 0, 45);
								$aCustomerData['buyer_name_last'] = substr($aOrderParams['customer']['shipment_last_name'], 0, 45);
								$aCustomerData['buyer_emailaddress'] = substr($aOrderParams['customer']['shipment_email'], 0, 150);
							}
							else
							{
							...
							if(isset($aOrderParams['products']) && is_array($aOrderParams['products']) && sizeof($aOrderParams['products']))
							{
								foreach($aOrderParams['products'] as $k => $v)
								{
									if(isset($v['code']) && isset($v['description']) && isset($v['quantity']) && isset($v['price_incl']))
									{
										...
										$aProduct['code'] = substr(preg_replace('/([^a-zA-Z0-9_\-]+)/', '_', $v['code']), 0, 50);
