 ...
 * Last Modified: 2014-05-09T00:49:26+02:00
...
class Xtento_StockImport_Model_Import_Entity_Stock extends Xtento_StockImport_Model_Import_Entity_Abstract
{
    ...
    private function _getCurrentPriceInfo()
    {
        ...
        if (self::$importCost) {
            $costAttributeId = Mage::getResourceModel('eav/entity_attribute')->getIdByCode('catalog_product', 'availability_notice');
            if ($costAttributeId) {
                $select = $this->_readAdapter->select()
...
if ($newPrice == "normaal leverbaar") {
  $newPrice = 31;
                        ...
                        if ($currentPrice !== $newPrice || $newPrice == '') {
                            Mage::getSingleton('catalog/product_action')
                                ->updateAttributes(array($productId), array('availability_notice' => $newPrice), Mage_Core_Model_App::ADMIN_STORE_ID);
                            // Cost has changed.
                    ...
                    if (isset($this->_costValues[$productId])) {
                        if (!empty($this->_costValues[$productId]['cost'])) {
                            Mage::getSingleton('catalog/product_action')
                                ->updateAttributes(array($productId), array('availability_notice' => ''), Mage_Core_Model_App::ADMIN_STORE_ID);
