...
class Mage_CatalogInventory_Model_Stock_Item extends Mage_Core_Model_Abstract
{
    ...
    public function checkQuoteItemQty($qty, $summaryQty, $origQty = 0)
    {
        ...
        if (!$this->checkQty($summaryQty) || !$this->checkQty($qty)) {
			...
			// EDDY added stock qty
			$message .= ' (voorraad is '.($this->getQty() * 1).')';
            $result->setHasError(true)
                ->setMessage($message)
            ...
            if (($this->getQty() - $summaryQty) < 0) {
                if ($this->getProductName()) {
					...
					// EDDY tbv custom text
					$_product= Mage::getSingleton('catalog/product')->load($this->getProductId());
					/*if ($_SERVER['REMOTE_ADDR'] == '95.97.2.79') {
						echo '<pre>';
						// print_r($_product->_data['stock_item']->_data['qty']);
						// print_r($this);
						echo $_product->getResource()->getAttribute('availability_notice')->getFrontend()->getValue($_product);
						echo '</pre>';
					}*/
                    ...
                    if ($this->getBackorders() == Mage_CatalogInventory_Model_Stock::BACKORDERS_YES_NOTIFY) {
                        if (!$this->getIsChildItem()) {
							if ($this->getQty() > 0) {
								$result->setMessage(
									Mage::helper('cataloginventory')->__('Voor het door u gewenste aantal bedraagt de %s. Wijzig voor directe levering van dit product het aantal in %s stuk(s) en klik op Winkelwagen bijwerken.', ( $_product->getResource()->getAttribute('availability_notice')->getFrontend()->getValue($_product) ), ( $this->getQty()*1 ) ) 
								);
							} else {
								$result->setMessage(
									...
									Mage::helper('cataloginventory')->__('%s.', ( $_product->getResource()->getAttribute('availability_notice')->getFrontend()->getValue($_product) ), ( $this->getQty()*1 ) ) 
								);
							}
                        } else {
                            $result->setMessage(
