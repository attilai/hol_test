...
class Mirasvit_Fpc_Model_Processor
{
    ...
    public function serveResponse()
    {
        ...
        if ($this->_storage->load()) {
            ...
            //smartwave proto compatibility
            if ($this->_checkoutMessage) {
                Mage::helper('fpc/smartwaveproto')->addCheckoutMessage($content, $this->_checkoutMessage);
    ...
    /**
     * Cache if for current request (md5)
     *
     * @return string
     */
    ...
    public function canProcessRequest($request = null)
    {
        ...
        //smartwave proto compatibility
        $this->_checkoutMessage = Mage::helper('fpc/smartwaveproto')->getCheckoutMessage();
        if (!$this->_checkoutMessage) {
            $messageTotal += Mage::getSingleton('checkout/session')->getMessages()->count();
        }
