		...
		if ($this->getConfigData('type'.$i) == 'O') { // per order
            ...
            {               
				...
				$coupon = Mage::getModel('salesrule/coupon');
 				$coupon->load(Mage::getSingleton('checkout/session')->getQuote()->getCouponCode(), 'code');
 				$couponObj = Mage::getModel('salesrule/rule')->load($coupon->getRuleId());
				//
				// Terra Preta aanpassing
				//
				if($couponObj->getName()=='Gratis verzending coupon' || $couponObj->getName()=='Gratis verzending coupon '){
	                $method->setPrice(0);
	                $method->setCost(0);
				} else {
